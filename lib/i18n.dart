class I18n {
  static final Map<String, Map<String, String>> _localizedValues = {
    'en': {
      'welcome': 'Welcome',
      'no_model_config':
          'No model configuration detected.\nPlease configure to use normally.',
      'start_config': 'Start Configuration',
      'no_status': 'No Status',
      'msg_empty': 'Current message is empty',
      'playing_voice': 'Playing voice...',
      'voice_gen_failed': 'Voice generation failed',
      'reply_completed': 'Reply completed',
      'draw_completed': 'Drawing completed',
      'gen_candidate_resp': 'Generating candidate responses',
      'select_reply': 'Select Response',
      'option': 'Option',
      'cancel': 'Cancel',
      'regenerate': 'Regenerate',
      'status_suffix': "'s Status",
      'edit_status': 'Edit Status',
      'clear': 'Clear',
      'confirm': 'Confirm',
      'edit': 'Edit',
      'refresh': 'Refresh',
      'add_to_chat': 'Add to Chat',
      'analyzing_status': 'Analyzing character status',
      'get_status_failed': 'Failed to get status',
      'reset': 'Reset',
      'saved': 'Saved',
      'no_messages': 'No messages.',
      'replying': 'Replying',
      'enter_message': 'Enter your message...',
      'status': 'Status',
      'draw': 'Draw',
      'auto_voice': 'Auto Voice',
      'auto_draw': 'Auto Draw',
      'manual_draw': 'Manual Draw',
      'manual_voice': 'Manual Voice',
      'auto_inspire': 'Auto Inspire',
      'manual_inspire': 'Manual Inspire',
      'auto_status': 'Auto Status',
      'manual_status': 'Manual Status',
      'exit_fullscreen': 'Exit Fullscreen',
      'enter_fullscreen': 'Enter Fullscreen',
      'story_list': 'Story List',
      'story_operations': 'Story Operations',
      'import_story': 'Import Story',
      'story_imported': 'Story imported successfully',
      'export_story': 'Export Story',
      'no_current_story': 'No current story set',
      'story_exported': 'Story exported',
      'current_story': 'Current Story',
      'story_unloaded': 'Current story unloaded',
      'story_pool': 'Story Pool',
      'set_as_current_story': 'Set ... as current story',
      'load_into_chat': 'Load into Chat',
      'delete': 'Delete',
      'delete_story': 'Delete Story',
      'delete_story_confirm': 'Are you sure you want to delete this story?',
      'character_list': 'Character List',
      'character_operations': 'Character Card Operations',
      'import_character': 'Import Character',
      'importing_character': 'Importing character...',
      'export_character': 'Export Character',
      'exporting_character': 'Exporting character...',
      'current_character': 'Current Character',
      'character_pool': 'Character Pool',
      'delete_character': 'Delete Character',
      'delete_character_confirm':
          'Are you sure you want to delete this character?',
      'settings': 'Settings',
      'general_settings': 'General Settings',
      'backup_config': 'Backup Config',
      'model_config': 'Model Config',
      'format_config': 'Format Config',
      'feature_settings': 'Feature Settings',
      'drawing_config': 'Drawing Config',
      'voice_config': 'Voice Config',
      'tts_backend': 'Voice Backend',
      'audio_cpp_server': 'audio.cpp Server URL',
      'audio_cpp_server_hint': 'For example: http://127.0.0.1:8080',
      'audio_cpp_model': 'audio.cpp Model ID',
      'audio_cpp_model_hint': 'The id configured in server.json',
      'audio_cpp_test_connection': 'Test connection and find models',
      'audio_cpp_connected': 'Connected; available models',
      'audio_cpp_no_models': 'no configured models',
      'audio_cpp_connect_failed': 'Connection failed',
      'sd_cpp_server': 'sd.cpp Server URL',
      'sd_cpp_server_hint': 'For example: http://127.0.0.1:1234',
      'sd_cpp_test_connection': 'Test connection and read server model',
      'sd_cpp_model': 'Model reported by sd-server',
      'sd_cpp_model_hint':
          'Determined by the sd-server -m argument; filled in by the connection test',
      'sd_cpp_no_image_mode':
          'The sd.cpp server does not support image generation',
      'sd_cpp_conn_failed': 'Cannot reach the sd.cpp server',
      'sd_cpp_connecting': 'Contacting the sd.cpp server...',
      'sd_cpp_job': 'Job submitted to the server',
      'sd_cpp_queued': 'Waiting in the server queue',
      'sd_cpp_generating': 'Generating image',
      'sd_cpp_sampler_unsupported':
          'Sampler is not supported by sd.cpp, using the server default',
      'sd_cpp_scheduler_ignored':
          'Scheduler is not supported by sd.cpp, ignored',
      'sd_cpp_lora': 'LoRA',
      'sd_cpp_lora_skipped':
          'LoRA is not in the server LoRA directory, skipped',
      'sd_cpp_size_clamped': 'Size adjusted to the server limits',
      'generation_queue': 'Queue & Logs',
      'generation_queue_running': 'running',
      'generation_queue_empty': 'No drawing or voice task yet.',
      'generation_queue_entries': 'Entries',
      'generation_queue_more': 'Older entries hidden',
      'generation_copy_log': 'Copy all logs',
      'generation_clear_log': 'Clear finished entries',
      'generation_log_copied': 'Logs copied to clipboard',
      'generation_kind_drawing': 'Drawing',
      'generation_kind_voice': 'Voice',
      'generation_status_queued': 'queued',
      'generation_status_running': 'running',
      'generation_status_completed': 'completed',
      'generation_status_failed': 'failed',
      'generation_status_cancelled': 'cancelled',
      'generation_error': 'Error',
      'generation_no_details': 'No details recorded',
      'generation_note_model': 'Model',
      'generation_note_job': 'Job',
      'generation_note_image': 'Image',
      'generation_note_voice_model': 'Voice model',
      'generation_note_reference': 'Reference voice',
      'generation_note_no_transcript': 'no transcript (x-vector-only mode)',
      'generation_note_audio_size': 'Audio size',
      'generation_note_request_count': 'Request',
      'test_connection': 'Test connection',
      'connection_success': 'Connection successful',
      'connection_failed': 'Connection failed',
      'about': 'About',
      'feedback': 'Feedback',
      'display_settings': 'Display Settings',
      'font_size': 'Font Size',
      'text_color': 'Text Color',
      'name_color': 'Name Color',
      'text_outline': 'Text Outline',
      'outline_width': 'Outline Width',
      'outline_color': 'Outline Color',
      'auto': 'Auto',
      'white': 'White',
      'black': 'Black',
      'red': 'Red',
      'blue': 'Blue',
      'gold': 'Gold',
      'pink': 'Pink',
      'light_mode': 'Light Mode',
      'dark_mode': 'Dark Mode',
      'chat': 'Chat',
      'story': 'Story',
      'role': 'Role',
      'language': 'Language',
      'prompt': 'Prompt',
      'generating_prompt': 'Generating prompt...',
      'regenerate_prompt': 'Regenerate prompt',
      'log': 'Log',
      'start': 'Start',
      'back': 'Back',
      'redraw': 'Redraw',
      'use': 'Use',
      'error': 'Error!',
      'preset_manage': 'Preset Management',
      'name': 'Name',
      'save_preset': 'Save Preset',
      'select_preset': 'Select Preset',
      'aidraw_prompt_llm': 'Aidraw Prompt LLM',
      'confirm_delete': 'Confirm Delete',
      'delete_confirm_msg': 'Are you sure you want to delete "?"?',
      'cannot_undo': 'This operation cannot be undone.',
      'please_fill_all': 'Please fill in all fields',
      'save_success': 'Saved successfully',
      'config_params': 'Configuration Parameters',
      'api_url': 'API URL (Web version only supports https)',
      'api_key': 'API Key',
      'model': 'Model',
      'temperature': 'Temperature',
      'frequency_penalty': 'Frequency Penalty (Optional)',
      'presence_penalty': 'Presence Penalty (Optional)',
      'max_output_length': 'Max Output Length',
      'common_prompt': 'Common Prompt',
      'user_name': 'User Name',
      'context_template': 'Context Template',
      'context_template_hint':
          'Hint: Use {{user}} for user name and {{char}} for character name in prompts.',
      'output_regex': 'Output Regex Filter',
      'edit_title': 'Edit ',
      'chat_prompt': 'Chat Prompt',
      'draw_prompt': 'Drawing Prompt',
      'status_prompt': 'Status Prompt',
      'inspire_prompt': 'Inspire Prompt',
      'welcome_prompt': 'Welcome Prompt',
      'summary_prompt': 'Summary Prompt',
      'msg_editor': 'Message Editor',
      'msg_content': 'Message Content',
      'save': 'Save',
      'select_all': 'Select All',
      'deselect_all': 'Deselect All',
      'add': 'Add',
      'voice': 'Voice',
      'resend': 'Resend',
      'confirm_resend': 'Confirm and Resend',
      'edit_system_instruction': 'Edit System Instruction',
      'turn_to_system_instruction': 'Convert to System Instruction',
      'remove': 'Remove',
      'set_background': 'Set as Background',
      'character_editor': 'Character Editor',
      'character_avatar': 'Character Avatar (Click to change)',
      'image_size_limit': 'Image size cannot exceed 1MB',
      'hint': 'Hint',
      'character_name': 'Character Name',
      'initial_dialogue': 'Initial Dialogue',
      'setting_prompt': 'Setting Prompt',
      'voice_ref': 'Voice Reference',
      'voice_ref_text': 'Voice Reference Transcript',
      'base_config': 'Base Config',
      'civitai_api_token': 'Civitai API Token',
      'tts_token_hint':
          'Used by Civitai voice synthesis; the drawing token is used as a fallback.',
      'tts_language': 'Speech Language',
      'emotion_params': 'Emotion Parameters',
      'happy': 'Happy',
      'sad': 'Sad',
      'angry': 'Angry',
      'afraid': 'Afraid',
      'disgusted': 'Disgusted',
      'melancholic': 'Melancholic',
      'surprised': 'Surprised',
      'calm': 'Calm',
      'sampler_config': 'Sampler Config',
      'sampler': 'Sampler',
      'width': 'Width',
      'height': 'Height',
      'steps': 'Steps',
      'cfg': 'CFG',
      'prompt_config': 'Prompt Config',
      'positive_prompt':
          'Positive Prompt (Enter VERB as action placeholder, CHAR as character placeholder)',
      'negative_prompt': 'Negative Prompt',
      'local_backup_restore': 'Local Backup & Restore',
      'download_config': 'Download Config',
      'file_restore': 'Restore from File',
      'confirm_restore': 'Confirm Restore',
      'restore_confirm_msg':
          'This will overwrite current configuration, continue?',
      'download_success': 'Download Successful',
      'download_failed': 'Download Failed',
      'restore_success': 'Restore Successful',
      'restore_failed': 'Restore Failed',
      'no_file_selected': 'No file selected',
      'webdav_backup_restore': 'WebDAV Backup & Restore (Not supported on Web)',
      'webdav_invalid_data': 'Invalid WebDAV URL or backup file.',
      'webdav_auth_failed': 'Authentication failed. Check your username and WebDAV app password.',
      'webdav_forbidden': 'This account does not have permission to access this directory.',
      'webdav_directory_missing': 'Remote directory not found. Check the URL or create the directory first. For Nutstore, you can use https://dav.jianguoyun.com/dav/.',
      'webdav_network_error': 'Connection failed or timed out. Check your network and server address.',
      'webdav_operation_failed': 'WebDAV operation failed. Please try again.',
      'webdav_backup_description': 'Restore this backup to replace the current configuration, or overwrite this remote file with the current configuration.',
      'username': 'Username',
      'password': 'Password',
      'test': 'Test',
      'backup': 'Backup',
      'no_records': 'No records',
      'overwrite': 'Overwrite',
      'naming_history': 'Name History',
      'generating': 'Generating...',
      'finish': 'Finish',
      'ai': 'AI',
      'decode_png_error': 'Error: Unable to decode PNG.',
      'found_chara_metadata': "Successfully found 'chara' metadata.",
      'decoded_json': 'Decoded JSON string: ',
      'decode_chara_error': "Error decoding 'chara' data: ",
      'chara_metadata_not_found': "Error: 'chara' metadata not found.",
      'json_empty': 'Error: JSON string is empty.',
      'import_character_book': 'Import Character Book',
      'import_character_book_msg':
          'Character book detected in character card, import as story?',
      'character_book_imported': 'Character book imported as story.',
      'download_card_error': 'Error downloading character card: ',
      'ping_ok': 'Ping OK',
      'ping_failed': 'Ping Failed: ',
      'backup_ok': 'Backup OK',
      'backup_failed': 'Backup Failed: ',
      'refresh_failed': 'Refresh Failed: ',
      'user': 'User',
      'assistant': 'Assistant',
      'system': 'System',
      'time': 'Time',
      'image': 'Image',
      'unknown': '?',
      'about_text':
          'MoeTalk is an open-source Flutter-based chat application that uses Large Language Models for dialogue generation and Diffusion Models for voice and image synthesis.\n\nYour data is stored entirely on your device; this application does not collect any personal information.\n\nCode repository: https://github.com/shinnpuru/MoeTalk',
      'default_user_name': 'Teacher',
      'default_student_name': 'Shinnpuru',
      'default_first_msg': 'Hello, I am Shinnpuru. How can I help you today?',
      'default_prompt':
          'You are an AI assistant named Shinnpuru. You can answer user questions, provide help and advice. Please communicate with the user in English.',
      'default_draw_prompt':
          'Your task is to generate a series of prompts based on {{char}}\'s current state to guide the diffusion model in generating images. The prompts should be a series of descriptive English words or phrases that guide the model to generate an image matching the description, specifically tags from the danbooru dataset. Separate prompts with commas, no newlines. Your response must only contain the image description, do not include any other explanations. The art style should be anime style, but you don\'t need to specify the style in the prompts. Do not include broad terms like 1girl, masterpiece. Example: blue sky, cake stand, capelet, chest harness, cloud, cloudy sky, cup, day, dress, flower, food, hair flower, hair ornament, harness, holding, holding cup, leaf, looking at viewer, neckerchief, chair, sitting, sky, solo, table. Please first summarize the current scene, perspective, composition, clothing, action, expression, etc., and then output the prompts after ||.',
      'default_inspire_prompt':
          'Based on the context, reply to {{char}} in one sentence using {{user}}\'s tone. Generate 3 candidate responses in different styles, separated by ||, without numbering.',
      'default_welcome_prompt':
          'Today is {{date}} {{time}}. As {{char}}, generate 3 creative welcome greetings based on the current date and time. Consider the time of day (morning/afternoon/evening/night), any special dates or holidays, and your character\'s personality. Output only the 3 greetings separated by ||, and \\ for breaking lines, without numbering or other text.',
      'default_status_prompt':
          'Briefly describe the character\'s current status, including affection level, clothing, actions, and inner thoughts. Affection level is out of 100. You can use markdown to draw tables.',
      'default_summary_prompt':
          'Based on the context, summarize the conversation in one sentence using {{char}}\'s tone.',
      'default_end_prompt':
          'Please act as {{char}} and reply with one or more paragraphs to advance the plot, using backslashes to separate them. You can use markdown, use italics for status.',
      'backend_type': 'Backend Type',
      'context_template_system':
          'The role-playing game begins now. You are {{char}}. Here is your character setting.',
      'context_template_description': 'charDescription',
      'context_template_world': 'Here is the world view.',
      'context_template_world_info': 'worldInfo',
      'context_template_history':
          'Here is the chat history between {{char}} and {{user}}.',
      'context_template_chat_history': 'chatHistory',
      'context_template_task': 'Here are the tasks to be completed.',
      'context_template_call_function': 'callFunction',
    },
    'zh': {
      'welcome': '欢迎使用',
      'no_model_config': '检测到您尚未配置模型\n为了正常使用，请先进行配置',
      'start_config': '开始配置',
      'no_status': '暂无状态',
      'msg_empty': '当前消息为空',
      'playing_voice': '正在播放语音...',
      'voice_gen_failed': '语音生成失败',
      'reply_completed': '回复完成',
      'draw_completed': '绘画完成',
      'gen_candidate_resp': '正在生成候选回复',
      'select_reply': '选择回复',
      'option': '选项',
      'cancel': '取消',
      'regenerate': '重新生成',
      'status_suffix': '的状态',
      'edit_status': '编辑状态',
      'clear': '清空',
      'confirm': '确定',
      'edit': '编辑',
      'refresh': '重新获取',
      'add_to_chat': '确认添加',
      'analyzing_status': '正在分析角色状态',
      'get_status_failed': '获取状态失败',
      'reset': '已重置',
      'saved': '已保存',
      'no_messages': '没有消息。',
      'replying': '回复中',
      'enter_message': '请输入您的消息...',
      'status': '查看状态',
      'draw': '绘图',
      'auto_voice': '自动语音',
      'auto_draw': '自动绘图',
      'manual_draw': '手动绘图',
      'manual_voice': '手动语音',
      'auto_inspire': '自动灵感',
      'manual_inspire': '手动灵感',
      'auto_status': '自动状态',
      'manual_status': '手动状态',
      'exit_fullscreen': '退出全屏',
      'enter_fullscreen': '进入全屏',
      'story_list': '故事列表',
      'story_operations': '故事操作',
      'import_story': '导入故事',
      'story_imported': '故事导入成功',
      'export_story': '导出故事',
      'no_current_story': '没有设置当前故事',
      'story_exported': '故事已导出',
      'current_story': '当前故事',
      'story_unloaded': '已卸载当前故事',
      'story_pool': '故事池',
      'set_as_current_story': '已将 ... 设置为当前故事',
      'load_into_chat': '加载到对话',
      'delete': '删除',
      'delete_story': '删除故事',
      'delete_story_confirm': '你确定要删除这个故事吗？',
      'character_list': '角色列表',
      'character_operations': '角色卡操作',
      'import_character': '导入角色',
      'importing_character': '正在导入角色...',
      'export_character': '导出角色',
      'exporting_character': '正在导出角色...',
      'current_character': '当前角色',
      'character_pool': '角色池',
      'delete_character': '删除角色',
      'delete_character_confirm': '你确定要删除这个角色吗？',
      'settings': '设置',
      'general_settings': '通用设置',
      'backup_config': '备份配置',
      'model_config': '模型配置',
      'format_config': '格式配置',
      'feature_settings': '功能设置',
      'drawing_config': '绘图配置',
      'voice_config': '语音配置',
      'tts_backend': '语音后端',
      'audio_cpp_server': 'audio.cpp Server 地址',
      'audio_cpp_server_hint': '例如：http://127.0.0.1:8080',
      'audio_cpp_model': 'audio.cpp 模型 ID',
      'audio_cpp_model_hint': '填写 server.json 中配置的 id',
      'audio_cpp_test_connection': '测试连接并获取模型',
      'audio_cpp_connected': '连接成功，可用模型',
      'audio_cpp_no_models': '没有已配置模型',
      'audio_cpp_connect_failed': '连接失败',
      'sd_cpp_server': 'sd.cpp 服务器地址',
      'sd_cpp_server_hint': '例如：http://127.0.0.1:1234',
      'sd_cpp_test_connection': '测试连接并读取服务器模型',
      'sd_cpp_model': 'sd-server 报告的模型',
      'sd_cpp_model_hint': '由 sd-server 的 -m 参数决定，测试连接后自动填充',
      'sd_cpp_no_image_mode': '该 sd.cpp 服务器不支持图像生成',
      'sd_cpp_conn_failed': '无法连接 sd.cpp 服务器',
      'sd_cpp_connecting': '正在连接 sd.cpp 服务器…',
      'sd_cpp_job': '任务已提交',
      'sd_cpp_queued': '排队中',
      'sd_cpp_generating': '正在生成图像',
      'sd_cpp_sampler_unsupported': 'sd.cpp 不支持该采样器，已使用服务器默认值',
      'sd_cpp_scheduler_ignored': 'sd.cpp 不支持该调度器，已忽略',
      'sd_cpp_lora': 'LoRA',
      'sd_cpp_lora_skipped': 'LoRA 不在服务器 LoRA 目录中，已跳过',
      'sd_cpp_size_clamped': '尺寸已按服务器限制调整',
      'generation_queue': '队列与日志',
      'generation_queue_running': '个进行中',
      'generation_queue_empty': '暂无绘图或语音任务',
      'generation_queue_entries': '记录数',
      'generation_queue_more': '更早的记录已隐藏',
      'generation_copy_log': '复制全部日志',
      'generation_clear_log': '清空已结束记录',
      'generation_log_copied': '日志已复制到剪贴板',
      'generation_kind_drawing': '绘图',
      'generation_kind_voice': '语音',
      'generation_status_queued': '排队中',
      'generation_status_running': '生成中',
      'generation_status_completed': '已完成',
      'generation_status_failed': '失败',
      'generation_status_cancelled': '已取消',
      'generation_error': '错误',
      'generation_no_details': '暂无详情记录',
      'generation_note_model': '模型',
      'generation_note_job': '任务',
      'generation_note_image': '图片',
      'generation_note_voice_model': '语音模型',
      'generation_note_reference': '参考语音',
      'generation_note_no_transcript': '无参考文本（x-vector-only 模式）',
      'generation_note_audio_size': '音频大小',
      'generation_note_request_count': '请求',
      'test_connection': '测试连接',
      'connection_success': '连接成功',
      'connection_failed': '连接失败',
      'about': '关于',
      'feedback': '反馈',
      'display_settings': '显示设置',
      'font_size': '字体大小',
      'text_color': '文字颜色',
      'name_color': '名字颜色',
      'text_outline': '文字描边',
      'outline_width': '描边宽度',
      'outline_color': '描边颜色',
      'auto': '自动',
      'white': '白色',
      'black': '黑色',
      'red': '红色',
      'blue': '蓝色',
      'gold': '金色',
      'pink': '粉色',
      'light_mode': '浅色模式',
      'dark_mode': '深色模式',
      'chat': '聊天',
      'story': '故事',
      'role': '角色',
      'language': '语言',
      'prompt': '提示词',
      'generating_prompt': '生成提示词中...',
      'regenerate_prompt': '重新生成提示词',
      'log': '日志',
      'start': '开始',
      'back': '返回',
      'redraw': '重绘',
      'use': '使用',
      'error': '出错了！',
      'preset_manage': '预设管理',
      'name': '名称',
      'save_preset': '保存预设',
      'select_preset': '选择预设',
      'aidraw_prompt_llm': '绘图提示词模型',
      'confirm_delete': '确认删除',
      'delete_confirm_msg': '您确定要删除 "?" 吗？',
      'cannot_undo': '此操作无法撤销。',
      'please_fill_all': '请填写所有字段',
      'save_success': '保存成功',
      'config_params': '配置参数',
      'api_url': 'API地址（网页版仅支持https链接）',
      'api_key': 'API密钥',
      'model': '模型',
      'temperature': '温度',
      'frequency_penalty': '频率惩罚（可选）',
      'presence_penalty': '存在惩罚（可选）',
      'max_output_length': '最大输出长度',
      'common_prompt': '通用提示词',
      'user_name': '用户名称',
      'context_template': '上下文模板',
      'context_template_hint': '提示：用户名称在提示词中可用{{user}}代替，角色名称可用{{char}}代替。',
      'output_regex': '输出正则过滤',
      'edit_title': '编辑 ',
      'chat_prompt': '聊天提示词',
      'draw_prompt': '绘画提示词',
      'status_prompt': '状态提示词',
      'inspire_prompt': '灵感提示词',
      'welcome_prompt': '欢迎语提示词',
      'summary_prompt': '总结提示词',
      'msg_editor': '消息编辑器',
      'msg_content': '消息内容',
      'save': '保存',
      'select_all': '全选',
      'deselect_all': '全不选',
      'add': '添加',
      'voice': '语音',
      'resend': '重发',
      'confirm_resend': '确定并重发',
      'edit_system_instruction': '编辑系统指令',
      'turn_to_system_instruction': '转为系统指令',
      'remove': '移除',
      'set_background': '设为背景',
      'character_editor': '角色编辑器',
      'character_avatar': '角色头像（点击更换）',
      'image_size_limit': '图片大小不能超过 1MB',
      'hint': '提示',
      'character_name': '角色名',
      'initial_dialogue': '初始对话',
      'setting_prompt': '设定提示词',
      'voice_ref': '语音参考',
      'voice_ref_text': '语音参考文本',
      'base_config': '基础配置',
      'civitai_api_token': 'Civitai API Token',
      'tts_token_hint': '用于 Civitai 语音合成；未填写时会沿用绘图 Token。',
      'tts_language': '语音语言',
      'emotion_params': '情感参数',
      'happy': '快乐',
      'sad': '悲伤',
      'angry': '愤怒',
      'afraid': '恐惧',
      'disgusted': '厌恶',
      'melancholic': '忧郁',
      'surprised': '惊讶',
      'calm': '平静',
      'sampler_config': '采样配置',
      'sampler': '采样器',
      'width': '宽度',
      'height': '高度',
      'steps': '步数',
      'cfg': 'CFG',
      'prompt_config': '提示词配置',
      'positive_prompt': '正向提示词(输入 VERB 作为动作占位符，输入 CHAR 作为角色占位符)',
      'negative_prompt': '负向提示词',
      'local_backup_restore': '本地备份与恢复',
      'download_config': '下载配置',
      'file_restore': '文件恢复',
      'confirm_restore': '确认恢复',
      'restore_confirm_msg': '此操作将覆盖当前配置，是否继续？',
      'download_success': '下载成功',
      'download_failed': '下载失败',
      'restore_success': '恢复成功',
      'restore_failed': '恢复失败',
      'no_file_selected': '未选择文件',
      'webdav_backup_restore': 'WebDAV 备份与恢复（网页版不支持此功能）',
      'webdav_invalid_data': 'WebDAV 地址或备份文件格式无效。',
      'webdav_auth_failed': '认证失败，请检查用户名和 WebDAV 应用密码。',
      'webdav_forbidden': '当前账号没有访问此目录的权限。',
      'webdav_directory_missing': '远程目录不存在，请检查地址或先创建对应文件夹。坚果云可使用 https://dav.jianguoyun.com/dav/。',
      'webdav_network_error': '连接失败或超时，请检查网络和服务器地址。',
      'webdav_operation_failed': 'WebDAV 操作失败，请重试。',
      'webdav_backup_description': '恢复此备份将替换当前配置；选择覆盖将用当前配置替换此远程文件。',
      'username': '用户名',
      'password': '密码',
      'test': '测试',
      'backup': '备份',
      'no_records': '无记录',
      'overwrite': '覆盖',
      'naming_history': '命名历史',
      'generating': '生成中...',
      'finish': '完成',
      'ai': 'AI',
      'decode_png_error': '错误: 无法解码 PNG。',
      'found_chara_metadata': "成功找到 'chara' 元数据。",
      'decoded_json': '解码后的 JSON 字符串: ',
      'decode_chara_error': "解码 'chara' 数据时出错: ",
      'chara_metadata_not_found': "错误: 未找到 'chara' 元数据。",
      'json_empty': '错误: JSON 字符串为空。',
      'import_character_book': '导入角色书',
      'import_character_book_msg': '检测到角色卡中包含角色书，是否将其作为故事导入？',
      'character_book_imported': '角色书已作为故事导入。',
      'download_card_error': '下载角色卡时出错: ',
      'ping_ok': 'Ping OK',
      'ping_failed': 'Ping Failed: ',
      'backup_ok': 'Backup OK',
      'backup_failed': 'Backup Failed: ',
      'refresh_failed': 'Refresh Failed: ',
      'user': '用户',
      'assistant': '角色',
      'system': '系统',
      'time': '时间',
      'image': '图像',
      'unknown': '?',
      'about_text':
          'MoeTalk 是一个基于Flutter的开源聊天应用，使用大语言模型进行对话生成，使用扩散模型进行语音和图像合成。\n\n您的数据完全存储在您的设备上，本应用不会收集任何个人信息。\n\n代码地址：https://github.com/shinnpuru/MoeTalk',
      'default_user_name': '老师',
      'default_student_name': '昕蒲',
      'default_first_msg': '你好，我是昕蒲。请问有什么可以帮助你的吗？',
      'default_prompt': '你是一个AI助手，名叫昕蒲。你可以回答用户的问题，提供帮助和建议。请使用中文与用户交流。',
      'default_draw_prompt':
          '你的任务是根据当前{{char}}的状态，生成一系列提示词，以指导扩散模型生成图像。提示词应该是一系列描述性的英语单词或短语，能够引导模型生成符合描述的图像，具体来说，是danbooru数据集中的标签。提示词用逗号分隔，没有换行。你的回复必须仅包含图片描述，不要包含任何其他说明等内容。画风应该是二次元风格，但不需要在提示词中写明画风。不要加入1girl, masterpiece等过于宽泛的词汇。示例：blue sky, cake stand, capelet, chest harness, cloud, cloudy sky, cup, day, dress, flower, food, hair flower, hair ornament, harness, holding, holding cup, leaf, looking at viewer, neckerchief, chair, sitting, sky, solo, table。请先总结当前的场景、视角、构图、服装、动作、表情等描述画面的详细内容，然后在||后面输出提示词。',
      'default_inspire_prompt':
          '根据上下文，以{{user}}的口吻用一句话回复{{char}}。生成3个不同风格的候选回复，不需要标号，用||分隔。',
      'default_welcome_prompt':
          '今天是{{date}} {{time}}。请你扮演{{char}}，根据当前的日期和时间生成3个创意的欢迎问候语。考虑一天中的时段（早晨/下午/傍晚/夜晚）、是否有特殊日期或节日，以及角色的性格特点。只输出3个问候语，用||分隔不同问候语，用\\换行，不要标号或其他文字。',
      'default_status_prompt':
          '简要描述角色当前的状态，包括好感度、服装、动作、心里话，好感度满分为100分。你可以使用markdown语法绘制表格。',
      'default_summary_prompt': '根据上下文，以{{char}}的口吻用一句话总结该对话。',
      'default_end_prompt':
          '请你扮演{{char}}用一段或多段话回复并推进剧情，可以使用反斜杠来间隔。你可以使用markdown语法，斜体表示状态。',
      'backend_type': '后端类型',
      'context_template_system': '现在开始角色扮演游戏，你叫{{char}}，下面是你的角色设定。',
      'context_template_description': 'charDescription',
      'context_template_world': '下面是世界观。',
      'context_template_world_info': 'worldInfo',
      'context_template_history': '下面是{{char}}和{{user}}的对话历史。',
      'context_template_chat_history': 'chatHistory',
      'context_template_task': '下面是需要完成的任务。',
      'context_template_call_function': 'callFunction',
    },
  };

  static String locale = 'zh';

  static String t(String key) {
    return _localizedValues[locale]?[key] ?? key;
  }
}
